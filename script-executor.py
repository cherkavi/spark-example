...
def execute_spark_script(argument_holder):
    ...
    process = subprocess.Popen(["spark-shell","-i",file_name], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
class ArgumentHolder(dict):
    ...
    def parse_arguments(arguments):
        ...
        if len(sys.argv)<2:
            print("name of script should be present")
            sys.exit(1)
