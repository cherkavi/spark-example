...
class AverageTime extends UserDefinedAggregateFunction {
  ...
  override def bufferSchema: StructType = StructType(StructField("elements", ArrayType(IntegerType, false)) :: StructField("count", IntegerType) :: Nil)
  ...
  override def initialize(buffer: MutableAggregationBuffer): Unit = {
    buffer(0) = Array[Integer]()
    buffer(1) = 0
  }

  // next element in group
  override def update(buffer: MutableAggregationBuffer, input: Row): Unit = {    
    buffer(0) = buffer.getAs[Array[Integer]](0).toArray :+ input.getAs[Integer](0).toArray
    buffer(1) = buffer.getAs[Integer](1) + 1
  }

  // merge with another row ( from another RDD )
  override def merge(buffer1: MutableAggregationBuffer, buffer2: Row): Unit = {
    buffer1(0) = buffer1.getAs[Array[Integer]](0) ++ buffer1.getAs[Array[Integer]](0)
    buffer1(1) = buffer1.getAs[Integer](1) + buffer2.getAs[Integer](1)
  }

  // This is where you output the final value, given the final value of your bufferSchema.
  override def evaluate(buffer: Row): Double = {
  	val allElements = buffer.getAs[Array[Integer]](0)
    var sum:Double = 0
    for(each <- allElements){
      sum+=each
    }
    sum/buffer.getAs[Integer](1)
  }
}
